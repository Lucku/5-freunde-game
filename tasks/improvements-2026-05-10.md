...
- [ ] 198. `server/simulation/WaveManager` retirement. After phase 3h.2 the bridge path owns wave spawn entirely; `WaveManager` is still instantiated + referenced by some tests as a no-op. Delete the class + test refs once the dependencies clear.
- [ ] 199. `server/simulation/constants.js` mirror retirement. Server has its own 19-hero `BASE_HERO_STATS` + `UPGRADE_POOL` (split from browser `Constants.js` for ESM/CJS interop). Consolidate via ESM→CJS adapter or shared module so both consumers read from the same source of truth. Avoids drift when hero stats change.
