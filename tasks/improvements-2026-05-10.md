...
- [ ] 195. Per-session `runState` (retire singleton). `runState` is exported as a process-wide singleton (`RunState.js:export const runState = createRunState()`). Server-sim step 3 lands with `GameSession._resetEcsState()` zeroing slot counts on each `init()` — works for sequential sessions but means two concurrent online matches on the same server share enemy/projectile slot pools. Required before scaling past 1 match per server process. Estimate: 2 sessions. Tracked in `tasks/server-sim-step-3.md` §9.
...
- [ ] 197. `Boss.js` RNG migration (22 sites). Boss attack-pattern selection / projectile spawn angle / phase-transition timing all use `Math.random()`. Boss instances live in `runState.bossInstances` and DO affect gameplay. Required for boss-fight determinism in online coop. Estimate: 1 session.
