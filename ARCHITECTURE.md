...
| 2 (shipped) | UI `saveData` reader sweep — `UI/Achievements.js`, `UI/Collection.js`, `UI/HeroDetails.js`, `UI/Statistics.js`, `UI/SkillTree.js` all read through `window.gameContext.saveData`. Tuning numbers (`ENEMIES_PER_WAVE`, `BIOME_OBSTACLE_DENSITY`) already cleanly imported via `Constants.js` ESM — left as-is. |
| 3 (shipped) | Registry API expansion — `GameContext.registries` gains `getBiome(id)` / `getHero(type)` / `getEnemy(type)` / `getDLC(key)` / `callHero(type, method, ...args)` / `listDLCs()` convenience methods that collapse the repeated `&&` guard chain. Base UI readers migrated: `UI/LevelUp.js` (4 sites), `UI/Shop.js` (1), `UI/SkillTree.js` (2), `UI/MainMenu.js` (1). 29 DLC files / 53 raw `HERO_LOGIC[…]` reads inside `dlc/` deferred — DLCs run server-side via `require()` through `server/simulation/loader.js`, so any DLC sweep must validate the server-stub layer. Migration is incremental there. |
| 4 (shipped, partial) | First ownership flip — added `_ownedDescriptor(backingKey, winAlias)` helper that backs the value on `GameContext` itself + re-defines `window.<winAlias>` as a getter pointing at the backing field. Flipped 3 single-writer properties: `canvas`, `ctx`, `defaultSaveData`. Writers in `game.js` now assign via `window.gameContext.X = …`; DLC bare-reads (`window.canvas`/`window.ctx`/`window._defaultSaveData`) still work — the legacy global is a reverse alias now. Multi-writer properties (`gameConfig`, `saveData`, entity arrays) deferred to session 5 once writers are audited. |
| 5 (shipped) | Final multi-writer flip — `gameConfig` (Config.js init writer, 5 mutate sites all in-place via `Object.assign` / deep-prop) and `saveData` (init declaration + `loadGame` result + `importSave` callback + `loadGame` fallback in game.js, plus `CloudSaveManager` merge writer) flipped to GameContext ownership. Object identity preserved across mutations because Config.js + game.js never reassign; only Cloud / fresh-init / import / loadGame swap whole-objects, and those route through the GameContext setter. Reverse aliases on `window.gameConfig` / `window.saveData` keep DLC + UI bare-reads working. Entity arrays (`enemies`/`projectiles`/`wave`/`arena`/`_world`) stayed delegated — coupled to #11 RunState carve which is a separate multi-session arc. |
