...
### Fixed
- **Online: smooth multiplayer rendering** — replaced direct snapshot-position application with a 4-entry ring-buffer interpolation system (`_onlineInterpBuf`). Remote player, enemies, and projectiles are now rendered 80 ms behind real-time, interpolating between two buffered snapshots instead of jumping to the latest server position every 50 ms. New ghost entities and single-snapshot startup still hard-snap as before; fallback extrapolation keeps rendering smooth while the buffer fills. Ghost enemies now show a brief white hit-flash when their HP drops between snapshots, giving immediate tactile feedback for projectile impacts without waiting for the server death event.
