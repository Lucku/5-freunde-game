...
### Changed
- **Online latency**: Global Lobby player-move updates throttle reduced from 100 ms to 50 ms (doubling update frequency); angle change threshold tightened from 0.09 to 0.05 rad for smoother remote-player rotation. In-game host snapshots increased from every 3 frames (~20 fps) to every 2 frames (~30 fps).
