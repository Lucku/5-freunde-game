...
### Added
- **World-context refactor — Phase 7 (parity test + CI)**: Added `server/simulation/parityTest.js` — 80-assertion test suite covering session isolation (no global state leakage between concurrent sessions), player movement direction, all 17 hero HP at init, enemy spawning, snapshot schema, delta-compression correctness, DLC hero smoke (all heroes × 5 ticks), and level-up flow (XP → LEVEL_UP message → choice → upgrade applied). Added `"test": "node server/simulation/parityTest.js"` to `package.json`. Added `.github/workflows/test.yml` — runs `npm test` on every push and pull request. "Remove duplicate physics" deferred: `WaveManager` creates plain objects, not `Enemy` instances, so `enemy.update()` cannot yet replace `_updateEnemies()` without a WaveManager refactor.
